...
const fail = round => ({
  ...
  calls: [
    { name: 'bash', input: { command: 'npm test', description: 'Run the tests' } },
...
await setScenario([
  { match: { title: 'UI', role: 'C', v: 1 }, calls: [
    ...
    { name: 'write', input: { file_path: '{dir}/requirement.md', content: '# 需求 v1\n\n- **FR-1**：实现 `add(a, b)`。\n- **AC-1**：运行 npm test 看到 ok。\n' } },
  ...
  { match: { title: 'UI', role: 'X', round: 1 }, delayMs: 2500, calls: [
    ...
    { name: 'bash', input: { command: 'npm test', description: 'Run the tests' } },
    { name: 'bash', input: { command: 'node --version && ls -la src', description: '确认运行时版本与产物' } },
